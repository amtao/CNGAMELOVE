...
#################打包android版本
def _packHot(txtServerlist,txtPf,txtBysdk,txtApipath,txtVersion,channel,isUpload,area):
	...
	if channel == ChannelEnum.Develop:
		bucketName = config[area]['cos_bucket_name_develop']
		region = config[area]['cos_region_develop']
	elif channel == ChannelEnum.Online:
		bucketName = config[area]['cos_bucket_name_online']
		region = config[area]['cos_region_online']
