...
import packAndTest,packHot,packApk
...
class PackUI:
	...
	def __init__(self,frame):
		self.wframe = frame
	############################ UI ##############################
	def createUI(self,state,channel=ChannelEnum.Develop):
		...
		if state == SysEnum.web:
			...
			if channel == ChannelEnum.Develop:
				title = "发布服热更包"
				serverList = config['develop_path']+"serverlist.php"
				apiPath = config['develop_path']+"version.php"
			elif channel == ChannelEnum.Online:
				title = "正式服热更包"
				serverList = config['online_path']+"serverlist.php"
				apiPath = config['online_path']+"version.php"
			elif channel == ChannelEnum.Test:
				title = "对外体验服热更包"
				serverList = config['develop_path2']+"serverlist.php"
				apiPath = config['develop_path2']+"version.php"
			version = "1.0.0"
			...
			self.lBysdk = 'login_by_sdk: !0,'
			title = "正式服ios包"
			serverList = config['ios_online_path']+"serverlist.php"
			apiPath = config['ios_online_path']+"version.php"
			version = "1.0.0"
		...
		if state == SysEnum.web:
			...
			txtBanben = ""
			if channel == ChannelEnum.Develop or channel == ChannelEnum.Test:
				txtBanben = "版本号(格式:1.0.0.xxx):"
			elif channel == ChannelEnum.Online:
				txtBanben = "版本号(格式:1.0.14.xxx):"
			verLabel1 = tk.Label(frame1,text = txtBanben,font =('Arial 10'))
	...
	def packHot2(self):
		...
		if self.state == SysEnum.hot:
			packHot._packHot(self.lServerlist,self.lPf,self.lBysdk,self.lApipath,self.verLabel2.get(),self.channel,self.cosType.get())
		else:
	...
	def packApk2(self):
		self.lVersion = 'version: "'+self.verLabel2.get()+'",'
		packApk._packApk(self.lServerlist,self.lPf,self.lBysdk,self.lApipath,self.verLabel2.get(),self.qudaoType.get(),self.apkVerLabel2.get(),self.apkType2.get())
