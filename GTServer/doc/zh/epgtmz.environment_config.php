...
define('MEMHOST', '172.17.4.124');// memcache host
define('MEMPORT', '9101');// memcache port
define('MEMUSERNAME', 'epgtmz1'); //此处要改（mem的名字）
define('MEMPASS', '');// memcache pass
define('REDISHOST', '172.17.4.3');// redis host
define('REDISPORT', '6379');// redis port
define('REDISPASS', 'play4fun88ylyz');// redis pass
define('DBHOST', '172.17.0.14');// db host
define('DBPORT', '3306');// db port
define('DBUSER', 'root');// db user
define('DBPASSWD', '%wUN$c6o&FVR');// db password
define('DBNAME', 's' . SERVER_ID . '_epgtmz');// db name
define('PREFIX_KEY', 'EPGTMZ' . 'S' . SERVER_ID . 's');// 缓存前缀
return array(
	// 路由配置文件1
	...
	'config.php' => array(
		'define' => array(
			'IN_INU' => 'true',
			'SYNC_W' => 'true',// 直接写入数据库
			'GAME_MARK' => "'epgtmz'",// 游戏标记
			'AGENT_CHANNEL_ALIAS' => "'EPGTMZ'",// 区分渠道标识
			'LOG_PATH' => "'/data/logs/epgtmz_log/' . date('Ymd') . '/'",// 日志目录
			'DOMAIN_HOST' => "'game-epgtmz.id-g.com'",// 入口域名
			'USE_PHPLOCK' => 'true',// 开启文件锁
			'LOCK_PATH' => "'/data/logs/epgtmz_log/'",// 文件锁日志目录
			'FILE_PATH' => "'/data/logs/'",//

			'AGENT_CHANNEL_NAME' => "'宫廷密传IOS专服'",// 渠道名称, 作用于后台管理显示
