...
define('MEMUSERNAME', 'grqnejq1'); //此处要改（mem的名字）
...
define('DBHOST', 'rm-wz998sjr34l25rkzt.mysql.rds.aliyuncs.com');// db host
define('DBPORT', '3306');// db port
define('DBUSER', 'grqnejq');// db user
define('DBPASSWD', 'grqnejq@zhisnet');// db password
define('DBNAME', 's' . SERVER_ID . '_grqnejq');// db name
define('PREFIX_KEY', 'GRQNEJQ' . 'S' . SERVER_ID . 's');// 缓存前缀
return array(
	// 路由配置文件1
	...
	'config.php' => array(
		'define' => array(
			...
			'GAME_MARK' => "'grqnejq'",// 游戏标记
			'AGENT_CHANNEL_ALIAS' => "'GRQNEJQ'",// 区分渠道标识
			'LOG_PATH' => "'/data/logs/grqnejq_log/' . date('Ymd') . '/'",// 日志目录
			'DOMAIN_HOST' => "'grqnejq.commpad.cn'",// 入口域名
			'USE_PHPLOCK' => 'true',// 开启文件锁
			'LOCK_PATH' => "'/data/logs/grqnejq_log/'",// 文件锁日志目录
			'FILE_PATH' => "'/data/logs/'",//

			'AGENT_CHANNEL_NAME' => "'官人去哪儿鲸旗'",// 渠道名称, 作用于后台管理显示
