...
define('MEMHOST', '10.66.97.26');// memcache host
define('MEMPORT', '9101');// memcache port
define('REDISHOST', '10.66.97.29');// redis host
define('REDISPORT', '6379');// redis port
define('REDISPASS', 'crs-h8yx1cgx:lyjx6kw@zhisnet');// redis port
define('DBHOST', '10.66.97.33');// db host
define('DBPORT', '3306');// db port
define('DBUSER', 'lyjx6kw');// db user
define('DBPASSWD', 'lyjx6kw@zhisnet');// db password
define('DBNAME', 's' . SERVER_ID . '_lyjx6kw');// db name
define('PREFIX_KEY', 'LYJX6KW' . 'S' . SERVER_ID . 's');// 缓存前缀
return array(
	// 路由配置文件1
	...
	'config.php' => array(
		'define' => array(
			...
			'GAME_MARK' => "'lyjx6kw'",// 游戏标记
			'AGENT_CHANNEL_ALIAS' => "'LYJX6KW'",// 区分渠道标识
			'LOG_PATH' => "'/data/logs/lyjx6kw_log/' . date('Ymd') . '/'",// 日志目录
			'DOMAIN_HOST' => "'lyjx6kw.zhisnet.cn'",// 入口域名
			'USE_PHPLOCK' => 'true',// 开启文件锁
			'LOCK_PATH' => "'/data/logs/lyjx6kw_log/'",// 文件锁日志目录
			'FILE_PATH' => "'/data/logs/'",// 

			'AGENT_CHANNEL_NAME' => "'老爷吉祥6kw'",// 渠道名称, 作用于后台管理显示
