...
define('MEMHOST', '10.66.239.226');// memcache host
define('MEMPORT', '9101');// memcache port
define('REDISHOST', '10.66.219.114');// redis host
define('REDISPORT', '6379');// redis port
define('REDISPASS', 'crs-h02l225p:gjjpch@zhisnet');// redis port
define('DBHOST', '10.66.239.169');// db host
define('DBPORT', '3306');// db port
define('DBUSER', 'gjjpch');// db user
define('DBPASSWD', 'gjjpch@zhisnet');// db password
define('DBNAME', 's' . SERVER_ID . '_gjjpch');// db name
define('PREFIX_KEY', 'GJJPCH' . 'S' . SERVER_ID . 's');// 缓存前缀
return array(
	// 路由配置文件1
	...
	'config.php' => array(
		'define' => array(
			...
			'GAME_MARK' => "'gjjpch'",// 游戏标记
			'AGENT_CHANNEL_ALIAS' => "'GJJPCH'",// 区分渠道标识
			'LOG_PATH' => "'/data/logs/gjjpch_log/' . date('Ymd') . '/'",// 日志目录
			'DOMAIN_HOST' => "'gjjpch.zhisnet.cn'",// 入口域名
			'USE_PHPLOCK' => 'true',// 开启文件锁
			'LOCK_PATH' => "'/data/logs/gjjpch_log/'",// 文件锁日志目录
			'FILE_PATH' => "'/data/logs/'",// 

			'AGENT_CHANNEL_NAME' => "'官居几品草花'",// 渠道名称, 作用于后台管理显示
