...
define('SERVER_ID', '2');// server id
define('MEMHOST', '10.66.172.124');// memcache host
define('MEMPORT', '9101');// memcache port
define('REDISHOST', '10.66.175.254');// redis host
define('REDISPORT', '6379');// redis port
define('REDISPASS', 'crs-4yg4fx0r:king@youdong');// redis port
define('DBHOST', '10.66.248.19');// db host
define('DBPORT', '3306');// db port
define('DBUSER', 'king');// db user
define('DBPASSWD', 'king@youdong');// db password
define('DBNAME', 's' . SERVER_ID . '_king');// db name
define('PREFIX_KEY', 'KING' . 'S' . SERVER_ID . 's');// 缓存前缀
return array(
	// 路由配置文件1
	...
	'config.php' => array(
		'define' => array(
			...
			'GAME_MARK' => "'king'",// 游戏标记
			'AGENT_CHANNEL_ALIAS' => "'KING'",// 区分渠道标识
			'LOG_PATH' => "'/data/logs/king_log/' . date('Ymd') . '/'",// 日志目录
			'DOMAIN_HOST' => "'s2.king.coolnull.com'",// 入口域名
			'USE_PHPLOCK' => 'true',// 开启文件锁
			'LOCK_PATH' => "'/data/logs/king_log/'",// 文件锁日志目录
			'FILE_PATH' => "'/data/logs/'",// 

			'AGENT_CHANNEL_NAME' => "'老爷吉祥'",// 渠道名称, 作用于后台管理显示
