...
define('DBNAME', 's' . SERVER_ID . '_eplocal');// db name
define('PREFIX_KEY', 'EPLOCAL' . 'S' . SERVER_ID . 's');// 缓存前缀
return array(
	// 路由配置文件1
	...
	'config.php' => array(
		'define' => array(
			...
			'GAME_MARK' => "'eplocal'",// 游戏标记
			'AGENT_CHANNEL_ALIAS' => "'EPLOCAL'",// 区分渠道标识
			'LOG_PATH' => "'/data/logs/eplocal_log/' . date('Ymd') . '/'",// 日志目录
			'DOMAIN_HOST' => "'id-g.com:81'",// 入口域名
			'USE_PHPLOCK' => 'true',// 开启文件锁
			'LOCK_PATH' => "'/data/logs/eplocal_log/'",// 文件锁日志目录
			'FILE_PATH' => "'/data/logs/'",//

			'AGENT_CHANNEL_NAME' => "'宫斗2-内部测试服'",// 渠道名称, 作用于后台管理显示
