...
define('MEMHOST', '172.18.158.131');// memcache host
define('MEMPORT', '11213');// memcache port
define('REDISHOST', '172.18.158.131');// redis host
define('REDISPORT', '6381');// redis port
define('DBHOST', '172.18.158.131');// db host
define('DBPORT', '3306');// db port
define('DBUSER', 'datang');// db user
define('DBPASSWD', 'datang@zhisnet');// db password
define('DBNAME', 'datangpgshf');// db name// 's' . SERVER_ID . '_guaji'
define('PREFIX_KEY', 'DATANGPGSHFTest' . 'S' . SERVER_ID . 's');// 缓存前缀
return array(
	// 全局公共配置，定义常量
	'config.php' => array(
		'define' => array(
			'IN_INU' => 'true',
			'SYNC_W' => 'true',// 直接写入数据库
			'GAME_MARK' => "'datangpgshf'",// 游戏标记
			'AGENT_CHANNEL_ALIAS' => "'DATANGPGSHF'",// 区分渠道标识
			'LOG_PATH' => "'/data/logs/datangpgshf_log/' . date('Ymd') . '/'",// 日志目录
			'DOMAIN_HOST' => "'datangpgshf.test.commpad.cn'",// 入口域名
			'USE_PHPLOCK' => 'true',// 开启文件锁
			'LOCK_PATH' => "'/data/logs/datangpgshf_log/'",// 文件锁日志目录
			'FILE_PATH' => "'/data/logs/'",// 

			'AGENT_CHANNEL_NAME' => "'大唐苹果审核服'",// 渠道名称, 作用于后台管理显示
