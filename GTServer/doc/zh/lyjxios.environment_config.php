...
define('MEMHOST', '10.66.252.106');// memcache host
define('MEMPORT', '9101');// memcache port
define('REDISHOST', '10.66.252.112');// redis host
define('REDISPORT', '6379');// redis port
define('REDISPASS', 'crs-3qsehoh7:lyjxios@zhisnet');// redis port
define('DBHOST', '10.66.252.102');// db host
define('DBPORT', '3306');// db port
define('DBUSER', 'lyjxios');// db user
define('DBPASSWD', 'lyjxios@zhisnet');// db password
define('DBNAME', 's' . SERVER_ID . '_lyjxios');// db name
define('PREFIX_KEY', 'LYJXIOS' . 'S' . SERVER_ID . 's');// 缓存前缀
return array(
	// 路由配置文件1
	...
	'config.php' => array(
		'define' => array(
			...
			'GAME_MARK' => "'lyjxios'",// 游戏标记
			'AGENT_CHANNEL_ALIAS' => "'LYJXIOS'",// 区分渠道标识
			'LOG_PATH' => "'/data/logs/lyjxios_log/' . date('Ymd') . '/'",// 日志目录
			'DOMAIN_HOST' => "'lyjxios.zhisnet.cn'",// 入口域名
			'USE_PHPLOCK' => 'true',// 开启文件锁
			'LOCK_PATH' => "'/data/logs/lyjxios_log/'",// 文件锁日志目录
			'FILE_PATH' => "'/data/logs/'",// 

			'AGENT_CHANNEL_NAME' => "'老爷吉祥ios'",// 渠道名称, 作用于后台管理显示
