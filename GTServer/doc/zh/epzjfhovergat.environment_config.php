...
define('MEMHOST', '10.41.92.32');// memcache host
define('MEMPORT', '11211');// memcache port
define('MEMUSERNAME', 'epzjfhovergats1'); //此处要改（mem的名字）
define('MEMPASS', '');// memcache pass
define('REDISHOST', '10.41.172.189');// redis host
define('REDISPORT', '6379');// redis port
define('REDISPASS', 'Redisplay4fun88ylyz');// redis pass
define('DBHOST', '10.41.77.219');// db host
define('DBPORT', '3306');// db port
define('DBUSER', 'root');// db user
define('DBPASSWD', 'epzjfh@aUve8xAEmb3khUc');// db password
define('DBNAME', 's' . SERVER_ID . '_epzjfhovergat');// db name
define('PREFIX_KEY', 'EPZJFHOVERGAT' . 'S' . SERVER_ID . 's');// 缓存前缀
return array(
	// 路由配置文件1
	...
	'config.php' => array(
		'define' => array(
			...
			'GAME_MARK' => "'epzjfhovergat'",// 游戏标记
			'AGENT_CHANNEL_ALIAS' => "'EPZJFHOVERGAT'",// 区分渠道标识
			'LOG_PATH' => "'/data/logs/epzjfhovergat_log/' . date('Ymd') . '/'",// 日志目录
			'DOMAIN_HOST' => "'game-epzjfhovergat.id-g.com'",// 入口域名
			'USE_PHPLOCK' => 'true',// 开启文件锁
			'LOCK_PATH' => "'/data/logs/epzjfhovergat_log/'",// 文件锁日志目录
			'FILE_PATH' => "'/data/logs/'",//

			'AGENT_CHANNEL_NAME' => "'﻿紫禁繁花港澳正式服'",// 渠道名称, 作用于后台管理显示
