...
define('DBPASSWD', 'epzjfhovergatpgshf@wl');// db password
define('DBNAME', 's' . SERVER_ID . '_epzjfhovergatpgshf');// db name
define('PREFIX_KEY', 'epzjfhovergatpgshf' . 'S' . SERVER_ID . 's');// 缓存前缀
return array(
	// 路由配置文件1
	...
	'config.php' => array(
		'define' => array(
			...
			'GAME_MARK' => "'epzjfhovergatpgshf'",// 游戏标记
			'AGENT_CHANNEL_ALIAS' => "'EPZJFHOVERGATPGSHF'",// 区分渠道标识
			'LOG_PATH' => "'/data/logs/epzjfhovergatpgshf_log/' . date('Ymd') . '/'",// 日志目录
			'DOMAIN_HOST' => "'game-epzjfhovergatpgshf.id-g.com'",// 入口域名
			'USE_PHPLOCK' => 'true',// 开启文件锁
			'LOCK_PATH' => "'/data/logs/epzjfhovergatpgshf_log/'",// 文件锁日志目录
			'FILE_PATH' => "'/data/logs/'",//

			'AGENT_CHANNEL_NAME' => "'EP2-港澳台蘋果審核服'",// 渠道名称, 作用于后台管理显示
