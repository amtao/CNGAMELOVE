...
define('MEMHOST', '192.168.9.86');// memcache host
define('MEMPORT', '11211');// memcache port
define('MEMUSERNAME', 'epzjfh1'); //此处要改（mem的名字）
define('MEMPASS', '');// memcache pass
define('REDISHOST', '192.168.9.86');// redis host
define('REDISPORT', '6379');// redis port
define('REDISPASS', 'dimartsepzjfh@2018');// redis pass
define('DBHOST', 'rm-uf686yy2y2x4961l4.mysql.rds.aliyuncs.com');// db host
define('DBPORT', '3306');// db port
define('DBUSER', 'epzjfh');// db user
define('DBPASSWD', 'epzjfh@id-g.com');// db password
define('DBNAME', 's' . SERVER_ID . '_epzjfh');// db name
define('PREFIX_KEY', 'EPZJFH' . 'S' . SERVER_ID . 's');// 缓存前缀
return array(
	// 路由配置文件1
	...
	'config.php' => array(
		'define' => array(
			'IN_INU' => 'true',
			'SYNC_W' => 'true',// 直接写入数据库
			'GAME_MARK' => "'epzjfh'",// 游戏标记
			'AGENT_CHANNEL_ALIAS' => "'EPZJFH'",// 区分渠道标识
			'LOG_PATH' => "'/data/logs/epzjfh_log/' . date('Ymd') . '/'",// 日志目录
			'DOMAIN_HOST' => "'game-epzjfh.id-g.com'",// 入口域名
			'USE_PHPLOCK' => 'true',// 开启文件锁
			'LOCK_PATH' => "'/data/logs/epzjfh_log/'",// 文件锁日志目录
			'FILE_PATH' => "'/data/logs/'",//

			'AGENT_CHANNEL_NAME' => "'﻿资禁繁花删档测试服'",// 渠道名称, 作用于后台管理显示
