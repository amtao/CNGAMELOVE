...
define('MEMPORT', '11212');// memcache port
define('REDISHOST', '127.0.0.1');// redis host
define('REDISPORT', '6380');// redis port
define('REDISPASS', '');// redis pass
define('DBHOST', '127.0.0.1');// db host
define('DBPORT', '3306');// db port
define('DBUSER', 'root');// db user
define('DBPASSWD', 'dimarts209');// db password
define('DBNAME', 's' . SERVER_ID . '_epdevlocal');// db name
define('PREFIX_KEY', 'EPDEVLOCAL' . 'S' . SERVER_ID . 's');// 缓存前缀
return array(
	// 路由配置文件1
	...
	'config.php' => array(
		'define' => array(
			...
			'GAME_MARK' => "'epdevlocal'",// 游戏标记
			'AGENT_CHANNEL_ALIAS' => "'EPDEVLOCAL'",// 区分渠道标识
			'LOG_PATH' => "'/data/logs/epdevlocal_log/' . date('Ymd') . '/'",// 日志目录
			'DOMAIN_HOST' => "'id-g.com:85'",// 入口域名
			'USE_PHPLOCK' => 'true',// 开启文件锁
			'LOCK_PATH' => "'/data/logs/epdevlocal_log/'",// 文件锁日志目录
			'FILE_PATH' => "'/data/logs/'",//

			'AGENT_CHANNEL_NAME' => "'宫斗2-内部开发测试服'",// 渠道名称, 作用于后台管理显示
