...
define('DBPASSWD', 'dimarts208');// db password
define('DBNAME', 's' . SERVER_ID . '_datanglocal');// db name
define('PREFIX_KEY', 'DATANGLOCALTest' . 'S' . SERVER_ID . 's');// 缓存前缀
return array(
	// 全局公共配置，定义常量
	'config.php' => array(
		'define' => array(
			'IN_INU' => 'true',
			'SYNC_W' => 'true',// 直接写入数据库
			'GAME_MARK' => "'datanglocal'",// 游戏标记
			'AGENT_CHANNEL_ALIAS' => "'DATANGLOCAL'",// 区分渠道标识
			'LOG_PATH' => "'/data/logs/datanglocal_log/' . date('Ymd') . '/'",// 日志目录
			'DOMAIN_HOST' => "'www.datang.com'",// 入口域名
			'USE_PHPLOCK' => 'true',// 开启文件锁
			'LOCK_PATH' => "'/data/logs/datanglocal_log/'",// 文件锁日志目录
			'FILE_PATH' => "'/data/logs/'",//

			'AGENT_CHANNEL_NAME' => "'大唐内网测试服'",// 渠道名称, 作用于后台管理显示
