...
define('MEMHOST', '10.253.19.118');// memcache host
define('MEMPORT', '11214');// memcache port
define('REDISHOST', '10.253.19.118');// redis host
define('REDISPORT', '6382');// redis port
define('REDISPASS', '');// redis pass
define('DBHOST', '10.253.19.118');// db host
define('DBPORT', '3306');// db port
define('DBUSER', 'king');// db user
define('DBPASSWD', 'king@youdong');// db password
define('DBNAME', 's' . SERVER_ID . '_lyjxtime');// db name
define('PREFIX_KEY', 'LYJXTIME' . 'S' . SERVER_ID . 's');// 缓存前缀
return array(
	// 路由配置文件1
	...
	'config.php' => array(
		'define' => array(
			...
			'GAME_MARK' => "'lyjxtime'",// 游戏标记
			'AGENT_CHANNEL_ALIAS' => "'LYJXTIME'",// 区分渠道标识
			'LOG_PATH' => "'/data/logs/lyjxtime_log/' . date('Ymd') . '/'",// 日志目录
			'DOMAIN_HOST' => "'lyjxtime.tuziyouxi.com'",// 入口域名
			'USE_PHPLOCK' => 'true',// 开启文件锁
			'LOCK_PATH' => "'/data/logs/lyjxtime_log/'",// 文件锁日志目录
			'FILE_PATH' => "'/data/logs/'",//

			'AGENT_CHANNEL_NAME' => "'老爷吉祥时间测试'",// 渠道名称, 作用于后台管理显示
