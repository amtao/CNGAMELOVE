...
define('MEMUSERNAME', 'dtqmgdtgaqy1'); //此处要改（mem的名字）
...
define('DBUSER', 'dtqmgdtgaqy');// db user
define('DBPASSWD', 'dtqmgdtgaqy@zhisnet');// db password
define('DBNAME', 's' . SERVER_ID . '_dtqmgdtgaqy');// db name
define('PREFIX_KEY', 'DTQMGDTGAQY' . 'S' . SERVER_ID . 's');// 缓存前缀
return array(
	// 路由配置文件1
	...
	'config.php' => array(
		'define' => array(
			...
			'GAME_MARK' => "'dtqmgdtgaqy'",// 游戏标记
			'AGENT_CHANNEL_ALIAS' => "'DTQMGDTGAQY'",// 区分渠道标识
			'LOG_PATH' => "'/data/logs/dtqmgdtgaqy_log/' . date('Ymd') . '/'",// 日志目录
			'DOMAIN_HOST' => "'dtqmgdtgaqy.commpad.cn'",// 入口域名
			'USE_PHPLOCK' => 'true',// 开启文件锁
			'LOCK_PATH' => "'/data/logs/dtqmgdtgaqy_log/'",// 文件锁日志目录
			'FILE_PATH' => "'/data/logs/'",//
