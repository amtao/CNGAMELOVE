...
define('MEMPORT', '11273');// memcache port
define('REDISHOST', '10.104.30.229');// redis host
define('REDISPORT', '6439');// redis port
...
define('DBNAME', 'kingbhshf');// db name// 's' . SERVER_ID . '_guaji'
define('PREFIX_KEY', 'KINGBHSHFTest' . 'S' . SERVER_ID . 's');// 缓存前缀
return array(
	// 全局公共配置，定义常量
	'config.php' => array(
		'define' => array(
			'IN_INU' => 'true',
			'SYNC_W' => 'true',// 直接写入数据库
			'GAME_MARK' => "'kingbhshf'",// 游戏标记
			'AGENT_CHANNEL_ALIAS' => "'KINGBHSHF'",// 区分渠道标识
			'LOG_PATH' => "'/data/logs/kingbhshf_log/' . date('Ymd') . '/'",// 日志目录
			'DOMAIN_HOST' => "'kingbhshf.test.zhisnet.cn'",// 入口域名
			'USE_PHPLOCK' => 'true',// 开启文件锁
			'LOCK_PATH' => "'/data/logs/kingbhshf_log/'",// 文件锁日志目录
			'FILE_PATH' => "'/data/logs/'",// 

			'AGENT_CHANNEL_NAME' => "'皇帝版号审核服'",// 渠道名称, 作用于后台管理显示
