...
if (is_array($serverList)) {
    foreach ($serverList as $k => $v) {
        ...
        for ($i = 0; $i < $table_div; $i++) {
            ...
            $sql = "SELECT `uid`,`name`,`vip`,`lastlogin`,`platform` FROM {$table} WHERE `lastlogin`>1512057600 AND `lastlogin`<1514649600 AND `platform` IN ('anfenggsjp','anfengiosjjdc','anfengiosjsms','anfengiosjwzz')";
    ...
    <table>
        ...
        <?php foreach($data as $k => $val){?>
                <tr style="background-color:#f6f9f3">
                    ...
                    <td style="text-align:center;"><?php echo $val['platform']; ?></td>
