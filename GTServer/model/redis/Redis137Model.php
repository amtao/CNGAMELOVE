...
/**
 * 跨服亲密冲榜--区间 pk区服 单人排行榜 (单人为一个单位)  =>个人奖励
 */
class Redis137Model extends RedisBaseModel
{
	public $comment = "跨服亲密冲榜-单人";
	public $act = 'huodong_314_user';//活动标签
    protected $_server_type = 6;

    public $b_mol = "kuacbhuodong";//返回信息 所在模块
    public $b_ctrl = "userlovelist";//返回信息 所在控制器
    ...
    public function __construct($key = '')
    {
        ...
        $this->_with_decimal_denominator = Game::get_now() - $this->hd_cfg['info']['sTime'];
    ...
    public function back_data_my($member){
        ...
        $Sev_Cfg = Common::getSevidCfg(Game::get_sevid($member));//子服ID
        Master::back_data(0,$this->b_mol,"mykualoveRid",array(
            "rid"=>$rid,
            'uid' => $member,
            'serv' => $Sev_Cfg['he'],
            'score' => $score,
