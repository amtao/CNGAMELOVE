...
/**
 * 跨服势力冲榜--区间 pk区服 排行榜  (区服为单位)   =>   整个区奖励
 */
class Redis132Model extends RedisBaseModel
{
	public $comment = "跨服势力冲榜-区服";
	public $act = 'huodong_313_serv';//活动标签
    protected $_server_type = 6;

    public $b_mol = "kuacbhuodong";//返回信息 所在模块
    public $b_ctrl = "qufulist";//返回信息 所在控制器
    ...
    public function back_data_my($member){
        ...
        Master::back_data(0,$this->b_mol,"mykuaquRid",array(
            'uid' => $member,
            'serv' => $Sev_Cfg['he'],
            'rid' => $rid,
            'score' => $score,
