...
/**
 * 跨服亲密冲榜--区间 排行榜  (区服为单位)   =>   本次匹配期间
 */
class Redis140Model extends RedisBaseModel
{
	public $comment = "跨服亲密冲榜";
	public $act = 'huodong_314_pk';//活动标签

    protected $_server_type = 6;

    public $b_mol = "kuacbhuodong";//返回信息 所在模块
    public $b_ctrl = "pkqufulovelist";//返回信息 所在控制器
    ...
    public function __construct($key = '')  //$key : 服务器id
    {
        ...
        if(!empty($this->kua_allzhufu)){
            ...
            if($key != ''){
                $this->act .= '_'.$this->kua_allzhufu;  //  huodong_314_pk_合服id_活动id
            }
            //活动 - 榜单key
    ...
    public function comein_pk($inservid){
        ...
        if(!empty($this->kua_allzhufu)){
            $Redis139Model = Master::getRedis139();
            $score = $Redis139Model->zScore($inservid);
