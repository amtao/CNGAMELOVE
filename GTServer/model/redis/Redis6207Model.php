 ...
 * 新年活动 - 每日排行
 */
class Redis6207Model extends RedisBaseModel
{
    public $comment = "国力庆典 - 每日寻访次数排行";
    public $act = 'huodong_6207';//活动标签
    public $out_start = 1;//常规输出范围 从第几个开始 下标从1开始
    public $out_num = 100;//常规输出范围 要获取几个
    public $b_mol = "glqdhuodong";//返回信息 所在模块
    public $b_ctrl = "xunfanglist";//返回信息 所在控制器
    ...
    public function back_data_my($member){
        ...
        Master::back_data(0,$this->b_mol,"myXunFangRid",array(
            "myScorerank"=>$rid,
            'myScore' => $score,
            'myName' => $name,
        ));
