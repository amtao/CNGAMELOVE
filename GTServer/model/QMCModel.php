...
class QMC {
	...
	static private function mc_init() {
		...
		static $mc = array();
		if( empty( $mc[$SevidCfg['sevid']] ) )
		{
			$mc[$SevidCfg['sevid']] = Common::getCacheBySevId($SevidCfg['sevid']);
		}
		return $mc[$SevidCfg['sevid']];
	}
	/**
	 * mc 计数器,增加计数并返回新的计数
	 ...
	 * @return array
	 */
	static public function output( $key, $max=5000 ){
		$out = array();
		$mc = self::mc_init();
