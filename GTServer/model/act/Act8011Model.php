 ...
 * 活动8011
 */
class Act8011Model extends ActHDBaseModel
{
    public $atype = 8011;//活动编号
    public $comment = "贵人令";
    public $b_mol = "guirenling";//返回信息 所在模块
    public $b_ctrl = "guirenlinghuodong";//子类配置
    public $hd_id = 'huodong_8011';//活动配置文件关键字
    ...
    public function upLevel($num = 1){
        ...
        for ($i=0; $i < $num; $i++) { 
            ...
            $levelInfo = Game::getcfg_info("magnate_lv", $this->info["level"]);
        ...
        //贵人令排行
        $Redis8011Model = Master::getRedis8011($this->hd_cfg['info']['id']);
        $Redis8011Model->zIncrBy($this->uid,$addLv);
    ...
    public function get_rwd($id = 0){
        ...
        if ($id > 0) {
            ...
            $idInfo = Game::getcfg_info("magnate_rwd", $id);
            if ($idInfo["lv"] <= $level) {
                $rwds[] = $idInfo;
            }
        }else{
    ...
    public function paihang(){
        ...
        //个人排行榜
        $Redis8011Model = Master::getRedis8011($this->hd_cfg['info']['id']);
        $Redis8011Model->back_data();
        $Redis8011Model->back_data_my($this->uid);
    }
    ...
    public function setCurrencyTask($type,$num){
        $magnate_task = Game::getcfg('magnate_task');
        foreach($magnate_task as $v){
    ...
    public function upTask($id = 0, $num = 1){
        ...
        if ($id <= 0) {
            Game::crontab_debug($this->uid."  err2  id:".$id."  num:".$num, "act8011_upTask");
            return false;
        }
        ...
        $magnate_task = Game::getcfg('magnate_task');
        if (empty($magnate_task[$id])){
            Game::crontab_debug($this->uid."  err3  id:".$id."  num:".$num, "act8011_upTask");
            return false;
        }

        $taskInfo = $magnate_task[$id];
        ...
        if ($addExp > 0) {
            ...
            if ( $maxExp <= 0 || $weekExp < $maxExp) {
                ...
                $addLv = 0;
                $magnate_lv = Game::getCfg('magnate_lv');
        ...
        // 贵人令
        Game::cmd_other_flow($this->uid, 'act8011', 'upTask_8011', array($id => $num), 10006, $this->info["level"], $addExp, $this->info['exp']);
