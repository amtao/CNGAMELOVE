 ...
 * 活动313
 */
class Act313Model extends ActHDBaseModel
{
	public $atype = 313;//活动编号
	public $comment = "跨服势力冲榜";
	public $b_mol = "kuacbhuodong";//返回信息 所在模块
	public $b_ctrl = "kuashili";//子类配置
	public $hd_cfg ;//活动配置
    ...
    public function make_out(){
        ...
        if(self::get_state() == 2 && $this->info['get'] < 2 && $this->outf['type'] == 2){
            ...
            $Redis132Model = Master::getRedis132($this->hd_cfg['info']['id']);
            $serv_he = Common::getSevCfgObj($servid)->getHE();
            $rid = $Redis132Model->get_rank_id($serv_he);
            if(!empty($rid)){
                //获取最大区服
                $reList = array();
     ...
     */
     public function do_save($num){
        ...
        if( self::get_state() == 1 && $num > 0){
            ...
            $Redis131Model = Master::getRedis131($this->hd_cfg['info']['id']);
            $Redis131Model->zIncrBy($this->uid,$num);

            //区间 pk区服 排行榜  (区服为单位)   =>   整个区奖励
            $servid = Game::get_sevid($this->uid);
            $serv_he = Common::getSevCfgObj($servid)->getHE();
            $Redis132Model = Master::getRedis132($this->hd_cfg['info']['id']);
            $Redis132Model->zIncrBy($serv_he,$num);
    ...
    */
    ...
    public function back_data_UserRank(){
        ...
        //排行信息
        $Redis131Model = Master::getRedis131($this->hd_cfg['info']['id']);
        $Redis131Model->back_data();
        $Redis131Model->back_data_my($this->uid);
    ...
    public function back_data_QuRank(){
        ...
        //排行信息
        $Redis132Model = Master::getRedis132($this->hd_cfg['info']['id']);
        $Redis132Model->back_data();
        $Redis132Model->back_data_my($this->uid);
...
//        }
//        if( self::get_state() == 1 ||  self::get_state() == 2){
...
//        }
        ...
        if($Sev28Model->isSensitify($msg) === false){
            if(empty($Sev39Model->info[$this->uid])){//正常
                $Sev313Model = Master::getSev313($this->hd_cfg['info']['id'],$this->kua_zhufu['zhufu']);
                $Sev313Model->add_msg($this->uid,$msg);
            }else{
                Master::back_s(2);
            }
        }
    ...
    public function list_init(){
        $Sev313Model = Master::getSev313($this->hd_cfg['info']['id'],$this->kua_zhufu['zhufu']);
        $Sev313Model->list_init($this->uid);
    }
    /**
     * 跨服势力冲榜聊天 - 输出聊天
     */
    public function list_click(){
        $Sev313Model = Master::getSev313($this->hd_cfg['info']['id'],$this->kua_zhufu['zhufu']);
        $Sev313Model->list_click($this->uid);
    }
    ...
    public function list_history($id){
        $Sev313Model = Master::getSev313($this->hd_cfg['info']['id'],$this->kua_zhufu['zhufu']);
        $Sev313Model->list_history($this->uid,$id);
    }
