 ...
 * 特惠礼包
 */
class Act84Model extends ActHDBaseModel
{
	public $atype = 84;//活动编号

	public $comment = "双十一特惠礼包";
...
// 	}
	...
	public function make_out(){
		$outof = array();
		$outof['cft']['id'] = $this->hd_cfg['giftlist_info']['id'];
		$outof['cft']['title'] = $this->hd_cfg['giftlist_info']['title'];
		$outof['cft']['sTime'] = strtotime($this->hd_cfg['giftlist_info']['startTime']);
		$outof['cft']['eTime'] = strtotime($this->hd_cfg['giftlist_info']['endTime']);
		$outof['cft']['cd']['next'] = $this->hd_cfg['giftlist_info']['endTime'];
		$outof['cft']['cd']['label'] = 'hdgiftlistcd';
		foreach ($this->hd_cfg['giftlist'] as $id => $v){
			$val['id'] = $id;
			$val['name'] = $v['name'];
			$val['need'] = $v['need'];
			$val['items'] = $v['items'];
    ...
    public function getState(){
        $state = 0;  //活动未进行
        $startTime = strtotime($this->hd_cfg['giftlist_info']['startTime']);
        $endTime = strtotime($this->hd_cfg['giftlist_info']['endTime']);
