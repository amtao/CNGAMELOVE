 ...
 * 活动6206
 */
class Act6206Model extends ActHDBaseModel
{
    public $atype = 6206;//活动编号
    public $comment = "国力庆典-每日办差排行";
    public $b_mol = "";//返回信息 所在模块
    public $b_ctrl = "";//子类配置
    public $hd_id = 'huodong_6206';//活动配置文件关键字
    ...
    public function add($num){
        if( self::get_state() == 1 ){
            ...
            $Redis6206Model = Master::getRedis6206($this->_get_day_redis_id());
            $paihang = $Redis6206Model->zScore($this->uid);

            if ($hdscore > $paihang){
                $Redis6206Model->zAdd($this->uid, $hdscore);
