 ...
 * 双十一-每日充值
 */
class Act85Model extends ActBaseModel
{
	public $atype = 85;//活动编号
	public $comment = "双十一-每日充值";
	...
	public function getRwd($id) {
	    ...
	    //获取充值奖励信息
	    if(empty($this->hd_cfg['rwd'][$id-1])){
	        Master::error(ACT_HD_GIVE_MAX);
	    }
	    $this->info['rwd'] +=1;
	    
	    if(!empty($this->hd_cfg['rwd'][$id-1]['items'])){
	        foreach ($this->hd_cfg['rwd'][$id-1]['items'] as $ite){
