 ...
 * 活动6203
 */
class Act6203Model extends ActHDBaseModel
{
    public $atype = 6203;//活动编号
    public $comment = "国力庆典-每日银两消耗排行";
    public $b_mol = "";//返回信息 所在模块
    public $b_ctrl = "";//子类配置
    public $hd_id = 'huodong_6203';//活动配置文件关键字
    ...
    public function add($num){
        if( self::get_state() == 1 ){
            ...
            $Redis6203Model = Master::getRedis6203($this->_get_day_redis_id());
            $paihang = $Redis6203Model->zScore($this->uid);
            if ($hdscore > $paihang){
                $Redis6203Model->zAdd($this->uid, $hdscore);
