 ...
 * 腊八节活动
 */
class Act286Model extends ActHDBaseModel
{
	public $atype = 286;//活动编号
	public $comment = "腊八节活动";
	public $b_mol = "LabaDay";//返回信息 所在模块
	public $b_ctrl = "cfg";//子类配置
	public $hd_id = 'huodong_286';//活动配置文件关键字
	public $item_type = 'hd286';
	public $date;
	...
	public function play($id){
	    ...
	    $h = date('H',time());
	    if($h<9 || $h>22){
	        Master::error(HD286_TYPE8_TIME_LIMIT);
	    }
	    ...
	    $Sev72Model = Master::getSev72($this->hd_cfg['info']['id']);
	    if($this->hd_cfg['boss'] - $Sev72Model->info[$this->date] <=0){
	        Master::error_msg(HD286_TYPE8_KILL_END);
	        $Sev72Model->back_data();
	    }else{
    	    $Sev72Model->add($score);
    	    $Sev72Model->back_data();
    	    
    	    Master::back_win($this->b_mol,"damage","score",$score);
    	    
            //扣除仓库道具
    	    $Act138Model = Master::getAct138($this->uid);
    	    $Act138Model->sub_hdItems($id);
    	    
    	    //添加积分
    	    $Act139Model = Master::getAct139($this->uid);
    	    $Act139Model->add_score($score);
	    }
	}
	...
	public function buyone($id){
	    ...
	    $Act136Model = Master::getAct136($this->uid);
	    if($shop[$id]['is_limit'] == 1 && $shop[$id]['limit'] <= $Act136Model->info[$id]){
	...
	public function exchange($id){
	    ...
	    $Act137Model = Master::getAct137($this->uid);
	    if($exchange[$id]['is_limit'] == 1 && $exchange[$id]['limit'] <= $Act137Model->info[$id]){
	        Master::error(HD_TYPE8_EXCEED_LIMIT);
	    }
	    $Act139Model = Master::getAct139($this->uid);
	    $Act139Model->add_score($exchange[$id]['need'],'hfscore');
	    $items = $exchange[$id]['item'];//要兑换的信息
	    if(empty($items['kind'])){
	        $items['kind'] = 1;
	    }
	    $Act137Model->add_items($id);
	    Master::add_item($this->uid,$items['kind'],$items['id'],$items['count']);
	}
	...
	public function data_out(){
	    ...
	    //积分信息
	    $Act139Model = Master::getAct139($this->uid);
	    $Act139Model->back_data();
	    
		//获取商城列表
		$Act136Model = Master::getAct136($this->uid);
		$Act136Model->back_data();
		
		//获取兑换列表
		$Act137Model = Master::getAct137($this->uid);
		$Act137Model->back_data();

		//仓库信息
		$Act138Model = Master::getAct138($this->uid);
		$Act138Model->back_data();
		
		//日志
		$Sev73Model = Master::getSev73($this->hd_cfg['info']['id']);
		$Sev73Model->bake_data();
		
		//累计充值领取档次
		$Act140Model = Master::getAct140($this->uid);
		$Act140Model->back_data();
	...
	public function paihang(){
	    //个人排行榜
	    $Redis123Model = Master::getRedis123($this->hd_cfg['info']['id']);
	    $Redis123Model->back_data();
	    $Redis123Model->back_data_my($this->uid);
	    
	    //联盟排行榜
	    $Redis124Model = Master::getRedis124($this->hd_cfg['info']['id']);
	    $Redis124Model->back_data();
	    $Act40Model = Master::getAct40($this->uid);
	    $cid = $Act40Model->info['cid'];
	    if(!empty($cid)){
	        $Redis124Model->back_data_my($cid);
	    }
	}
	/*
	 * 击杀boss奖励
	...
	public function KillRwd(){
	    ...
	    if($this->hd_cfg['boss'] - $Sev72Model->info[$time] > 0){
	        Master::error(HD286_TYPE8_DONT_LINGQU);
	    }
	    //判断自己是否领取过
	    $Act139Model = Master::getAct139($this->uid);
	    if($Act139Model->info['kill'][$time]){
	        Master::error(HD_TYPE8_HAVE_LINGQU);
	    }
	    $Act139Model->addKillRwd($this->hd_cfg['kill_boss']);
	}
	...
	public function delClubScore($cid){
		...
		//获取当前用户在这个联盟的积分
		$Act139Model = Master::getAct139($this->uid);
		if(empty($Act139Model->info['hdscore'])) return;
		$Redis124Model = Master::getRedis124($this->hd_cfg['info']['id']);
		$Redis124Model->zIncrBy($cid,-$Act139Model->info['hdscore']);
		Game::cmd_other_flow($cid , 'club', 'huodong_286_'.$this->hd_cfg['info']['id'], array($this->uid), 43, 1, -$Act139Model->info['hdscore'], $Redis124Model->zScore($cid));
	}
	/*
	 * 新加联盟积分
	 * */
