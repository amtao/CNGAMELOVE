 ...
 * 活动8006
 */
class Act8006Model extends ActHDBaseModel
{
    public $atype = 8006;//活动编号
    public $comment = "厨艺大赛";
    public $b_mol = "chuyigame";//返回信息 所在模块
    public $b_ctrl = "chuyidasai";//子类配置
    public $hd_id = 'huodong_8006';//活动配置文件关键字
    ...
    public function play($num = 1, $score = 0){
        ...
        //每日排行
        $Redis8006Model = Master::getRedis8006($this->_get_day_redis_id());
        $Redis8006Model->zIncrBy($this->uid,$num);
    ...
    public function paihang($type){
        ...
        //个人排行榜
        $Redis8006Model = Master::getRedis8006($rankId);
        $Redis8006Model->back_data();
        $Redis8006Model->back_data_my($this->uid);
    }
