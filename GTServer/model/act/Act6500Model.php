 ...
 * 七天签到活动
 */
class Act6500Model extends ActHDBaseModel
{
	public $atype = 6500;//活动编号
	public $comment = "四十五天签到活动";
	public $b_mol = "fottyFiveSign";//返回信息 所在模块
	public $b_ctrl = "cfg";//子类配置
	public $hd_id = 'huodong_6500';//活动配置文件关键字
	public $item_type = 'hd6500';
	public $date;
    ...
    public function get_state(){
        ...
        if(!empty($this->hd_cfg)){
            ...
            if($_SERVER['REQUEST_TIME'] >= $startTime && $_SERVER['REQUEST_TIME'] <= $endTime && count($this->info['num']) < $maxDay){
                $state = 1;  //活动中
            }
        }
        return $state;
    ...
    public function sign(){
        ...
        if(($_SERVER['REQUEST_TIME'] - $sign) > 24*3600){
            ...
            $this->info['day'] = $this->info['day'] > $maxDay?$maxDay:$this->info['day'];//最大签到次数7次
            $this->save();
        }
        $this->data_out();
    }
    ...
    public function rwd($id){
        if($this->get_state() == 0){
            Master::error(FORTYFIVESIGN_MSG);
        }
        $maxDay = count($this->hd_cfg['rwd']);
        if($id < 1 || $id > $maxDay){
            Master::error(FORTYFIVESIGN_PARAMS_ERROR.$id);
        }
        if(in_array($id,$this->info['num'])){
            Master::error(FORTYFIVESIGN_REPEAT_RWD);
        }
        if($id > $this->info['day'])
        {
            Master::error(FORTYFIVESIGN_MORE_DAY);
        }
        ...
        if(empty($rwd)){
            Master::error(FORTYFIVESIGN_HD_CFG);
        }
        foreach ($rwd as $val){
