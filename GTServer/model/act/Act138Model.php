 ...
 * 腊八节活动 - 仓库
 */
class Act138Model extends ActBaseModel
{
    public $atype = 138;//活动编号
    public $b_mol = "LabaDay";//返回信息 所在模块
    public $b_ctrl = "bag";//返回信息 所在控制器
    public $comment = "腊八节活动 - 仓库";
    ...
    public function randreward($itemcfg_info,$num=1){
        ...
        $Sev73Model = Master::getSev73($hd_cfg['info']['id']);
        foreach($allitems as $ak => $av){
            ...
            if($type == 'rizhi'){
                $Sev73Model->add($this->uid, $itemcfg_info['id'], $ak,$av);
            }
