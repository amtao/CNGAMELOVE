 ...
 * 活动253
 */
class Act253Model extends ActHDBaseModel
{
	public $atype = 253;//活动编号
	public $comment = "亲密冲榜";
	public $b_mol = "cbhuodong";//返回信息 所在模块
	public $b_ctrl = "love";//子类配置
	public $hd_cfg ;//活动配置
	public $hd_id = 'huodong_253';//活动配置文件关键字
	...
	public function do_save($num){
		...
		if( parent::get_state() == 1){
            ...
            if($score - $this->info['num'] > 0){
                $Redis104Model = Master::getRedis104($this->hd_cfg['info']['id']);
                $Redis104Model->zAdd($this->uid,$score - $this->info['num']);
            }
	...
	public function back_data_hd(){
		...
		//排行信息
		$Redis104Model = Master::getRedis104($this->hd_cfg['info']['id']);
		$Redis104Model->back_data();
		$Redis104Model->back_data_my($this->uid);
     ...
     * 返回活动信息
     */
    public function back_data_hd314(){
