 ...
 * 活动8005
 */
class Act8005Model extends ActHDBaseModel
{
    public $atype = 8005;//活动编号
    public $comment = "圣诞节活动";
    public $b_mol = "sdjhuodong";//返回信息 所在模块
    public $b_ctrl = "christmas";//子类配置
    public $hd_id = 'huodong_8005';//活动配置文件关键字
    ...
    public function play($num = 1){
        ...
        //排行榜
        $Redis8005Model = Master::getRedis8005($this->hd_cfg['info']['id']);
        $Redis8005Model->zIncrBy($this->uid,$num);
    ...
    public function get_rwd($id = 0){
        ...
        $cons = $this->info['cons'];
        $rwds = Game::get_key2id($this->hd_cfg['rwd'],'cons');
        $itmes = $rwds[$id];
        if (empty($itmes)){
    ...
    public function paihang(){
        //个人排行榜
        $Redis8005Model = Master::getRedis8005($this->hd_cfg['info']['id']);
        $Redis8005Model->back_data();
        $Redis8005Model->back_data_my($this->uid);
    }
