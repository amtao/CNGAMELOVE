 ...
 * 腊八节活动 - 积分
 */
class Act139Model extends ActBaseModel
{
    public $atype = 139;//活动编号

    public $comment = "腊八节活动 - 积分";
    ...
    public function add_score($score,$type='hdscore')
    {
        ...
        if($type == 'hdscore'){
            ...
            $Redis123Model->zIncrBy($this->uid,$score);
            //添加联盟排名信息
            $Act40Model = Master::getAct40($this->uid);
            if(!empty($Act40Model->info['cid'])){
                $Redis124Model = Master::getRedis124($this->hd_cfg['info']['id']);
                $Redis124Model->zIncrBy($Act40Model->info['cid'],$score);
                Game::cmd_other_flow($Act40Model->info['cid'] , 'club', 'huodong_286_'.$this->hd_cfg['info']['id'], array($this->uid), 43, 1, $score, $Redis124Model->zScore($Act40Model->info['cid']));
            }
        }
    ...
    public function addClubScore($cid){
        ...
        $Redis124Model = Master::getRedis124($this->hd_cfg['info']['id']);
        $Redis124Model->zIncrBy($cid,$this->info['hdscore']);
        Game::cmd_other_flow($cid , 'club', 'huodong_286_'.$this->hd_cfg['info']['id'], array($this->uid), 43, 1, $this->info['hdscore'], $Redis124Model->zScore($cid));
        $this->save();
    }
    /*
     *迁移联盟记录数据
