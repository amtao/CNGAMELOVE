 ...
 * 惩戒来福-仓库
 */
class Act106Model extends ActBaseModel
{
	public $atype = 106;//活动编号
	public $b_mol = "penalize";//返回信息 所在模块
	public $b_ctrl = "bag";//返回信息 所在控制器
	public $comment = "惩戒来福-仓库";
	public $hd_cfg;
	...
	public function randreward($itemcfg_info,$num=1){
	    ...
	    $Sev37Model = Master::getSev37($hd_cfg['info']['id']);
	    foreach($allitems as $ak => $av){
	        ...
	        if($type == 'rizhi'){
	            $Sev37Model->add($this->uid, $itemcfg_info['id'], $ak,$av);
	        }
	        $reward[] = array('id'=>$ak,'count'=>$av,'kind'=>1);
	    }
	    if(!empty($reward)){
	        Master::back_win("penalize","reward","score",$score);
	        Master::back_win("penalize","reward","hdscore",$score);
	        Master::back_win("penalize","reward","items",$reward);
	    }
