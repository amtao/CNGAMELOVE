 ...
 * 活动270
 */
class Act270Model extends ActHDBaseModel
{
	public $atype = 270;//活动编号
	public $comment = "四大奸臣";
	public $b_mol = "jchuodong";//返回信息 所在模块
	public $b_ctrl = "jianchen";//子类配置
	public $hd_cfg ;//活动配置
	public $hd_id = 'huodong_270';//活动配置文件关键字
	...
	public function get_news(){
		...
		foreach($this->hd_cfg['rwd'] as $rwd){
			//验证道具是否充足
			$flag = $ItemModel->sub_item(130,$rwd['need'],true);
	...
	public function get_rwd($id = 0){
		...
		$ItemModel = Master::getItem($this->uid);
		$ItemModel->sub_item(130,$brwd[$id]['need'],true);
		
		//加门客
		Master::add_item($this->uid,KIND_HERO,$id);
		
		$HeroModel = Master::getHero($this->uid);
		$HeroModel->add_ghlevel($this->atype);

		//扣除道具
		$ItemModel->sub_item(130,$brwd[$id]['need']);
