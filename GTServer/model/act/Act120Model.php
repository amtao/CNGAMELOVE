 ...
 * 重阳节活动 - 仓库
 */
class Act120Model extends ActBaseModel
{
	public $atype = 120;//活动编号
	public $b_mol = "doubleNinth";//返回信息 所在模块
	public $b_ctrl = "bag";//返回信息 所在控制器
	public $comment = "重阳节活动 - 仓库";
	...
	public function randreward($itemcfg_info,$num=1){
	    ...
	    $Sev46Model = Master::getSev46($hd_cfg['info']['id']);
	    foreach($allitems as $ak => $av){
	        ...
	        if($type == 'rizhi'){
	            $Sev46Model->add($this->uid, $itemcfg_info['id'], $ak,$av);
	        }
