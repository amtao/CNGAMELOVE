 ...
 * 新官上任-仓库
 */
class Act102Model extends ActBaseModel
{
	public $atype = 102;//活动编号
	public $b_mol = "xghuodong";//返回信息 所在模块
	public $b_ctrl = "bag";//返回信息 所在控制器
	public $comment = "新官上任-仓库";
	public $hd_cfg;
	...
	public function randreward($itemcfg_info,$num=1){
	    ...
	    $Sev30Model = Master::getSev30($hd_cfg['info']['id']);
	    foreach($allitems as $ak => $av){
	        ...
	        if($type == 'rizhi'){
	            $Sev30Model->add($this->uid, $itemcfg_info['id'], $ak,$av);
	        }
	        $reward[] = array('id'=>$ak,'count'=>$av,'kind'=>1);
	    }
	    if(!empty($reward)){
	        Master::back_win("xghuodong","reward","score",$score);
	        Master::back_win("xghuodong","reward","hdscore",$score);
	        Master::back_win("xghuodong","reward","items",$reward);
	    }
