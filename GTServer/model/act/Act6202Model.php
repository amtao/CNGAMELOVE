 ...
 * 活动6202
 */
class Act6202Model extends ActHDBaseModel
{
    public $atype = 6202;//活动编号
    public $comment = "国力庆典-每日阅历消耗排行";
    public $b_mol = "";//返回信息 所在模块
    public $b_ctrl = "";//子类配置
    public $hd_id = 'huodong_6202';//活动配置文件关键字
    ...
    public function add($num){
        if( self::get_state() == 1 ){
            ...
            $Redis6202Model = Master::getRedis6202($this->_get_day_redis_id());
            $paihang = $Redis6202Model->zScore($this->uid);

            if ($hdscore > $paihang){
                $Redis6202Model->zAdd($this->uid, $hdscore);
