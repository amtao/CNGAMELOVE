 ...
 * 活动6207
 */
class Act6207Model extends ActHDBaseModel
{
    public $atype = 6207;//活动编号
    public $comment = "国力庆典-每日出城次数排行";
    public $b_mol = "";//返回信息 所在模块
    public $b_ctrl = "";//子类配置
    public $hd_id = 'huodong_6207';//活动配置文件关键字
    ...
    public function add($num){
        if( self::get_state() == 1 ){
            ...
            $Redis6207Model = Master::getRedis6207($this->_get_day_redis_id());
            $paihang = $Redis6207Model->zScore($this->uid);

            if ($hdscore > $paihang){
                $Redis6207Model->zAdd($this->uid, $hdscore);
