 ...
 * 活动6208
 */
class Act6208Model extends ActHDBaseModel
{
    public $atype = 6208;//活动编号
    public $comment = "国力庆典-每日联姻次数排行";
    public $b_mol = "";//返回信息 所在模块
    public $b_ctrl = "";//子类配置
    public $hd_id = 'huodong_6208';//活动配置文件关键字
    ...
    public function add($num){
        if( self::get_state() == 1 ){
            ...
            $Redis6208Model = Master::getRedis6208($this->_get_day_redis_id());
            $paihang = $Redis6208Model->zScore($this->uid);

            if ($hdscore > $paihang){
                $Redis6208Model->zAdd($this->uid, $hdscore);
