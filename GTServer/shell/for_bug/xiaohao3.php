...
function data(){
    ...
    $db = Common::getMyDb();
	$flow_db = Common::getMyDb('flow');
    for($i = 0;$i<100;$i++){
        if($i < 10){
            $table = 'user_0'.$i;
			$flow_table = 'flow_event_0'.$i;
        }else{
            $table = 'user_'.$i;
			$flow_table = 'flow_event_0'.$i;
        }
        $sql = "select `uid`,`name`,`level`,`vip`,`cash_buy`,`regtime`,`lastlogin`,`platform`,`ip` from {$table} where `lastlogin` > {$todayTime} and `level` < 7";
        $data[$table] = $db->fetchArray($sql);
		foreach($data[$table] as &$value){
			$sql = "select count(*) as num from {$flow_table} where `ftime` > {$todayTime} and `uid` = {$value['uid']};";
			$flow_data = $flow_db->fetchRow($sql);
			$value['num'] = $flow_data['num'];
		}
        echo mysql_error();
    }
