...
if(!empty($data) && is_array($data))
{
    ...
    {
        $url = "<a class='backGroundColor' href='?sevid={$SevidCfg['sevid']}&mod=config&act=addConfig&type=actNewConfig&key={$key}'>{$key}</a>";
        $delete = '';
        $delete = "<a class='backGroundColor' onclick='return confirm(\"您确定要删除吗(Are you sure?)\");' href='?sevid={$SevidCfg['sevid']}&mod=config&act=delConfig&type=actNewConfig&key={$key}'>删除</a>";
        echo "<tr><td>{$url}</td><td>".htmlspecialchars(substr($value,0,80))."</td><td>$delete</td></tr>";
    }
}
$addKey = '添加活动配置';
$url = "<a class='backGroundColor' href='?sevid={$SevidCfg['sevid']}&mod=config&act=addConfig&type=actNewConfig&key='>{$addKey}</a>";
$delete = '';
