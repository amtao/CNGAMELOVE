            ...
            foreach($data as $v){
                ...
                if (is_array($v['record'])){
                    foreach ($v['record'] as $rk => $rv){
                        ...
                        echo '</td><td>'.$gameConfig[$rv['type']]['title'].'</td>';
                        echo '<td>';
                        if(($rv['type'] <= 13 && $rv['type'] > 7) || $rv['type'] == 26 || $rv['type'] == 25){
                            ...
                            if (empty($itemConfig[$rv['itemid']]['name_cn'])){
                                echo '道具id:'.$rv['itemid'].'</td>';
                            }else{
                                echo $itemConfig[$rv['itemid']]['name_cn'].'</td>';
                            }
                        }else{
                            echo $rv['itemid'].'</td>';
                        }
                        if ($rv['type'] == 27){
                            foreach ($clubConfig as $ck => $cv){
                                if ($rv['next'] > $cv){
                                    $rv['next']=$rv['next']-$cv;
                                }else{
                                    break;
