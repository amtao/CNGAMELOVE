                ...
                <td style="text-align: left;">
                    ...
                    <select name="items" id="items" <?php if($_POST['type'] != 6) echo 'style="display:none"';?> >
                        ...
                        foreach($itemConfig as $v){
                            echo '<option value="'.$v['id'].'"'.($v['id'] == $_POST['items'] ? "selected" : '').'>'.$v['id'].'-'.$v['name'] . '</option>';
                        }
                        ?>
                    </select>
            ...
            foreach($data as $v){
                ...
                if (is_array($v['record'])){
                    foreach ($v['record'] as $rk => $rv){
                        ...
                        echo '('.$v['ctrl'].')</td><td style="width: 300px;">'.$v['params'].'</td><td>'.$gameConfig[$rv['type']]['title'].'</td>';
                        echo '<td>';
