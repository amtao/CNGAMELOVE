...
if($data){
    ...
    <table>
        ...
        <?php foreach($data as $k => $val){?>
            <tr <?php if (date('Y-m-d', strtotime($val['dateTime'])) == date('Y-m-d')){ echo 'style="background-color:#fe9b9b;"';}else{echo 'style="background-color:#f6f9f3;"'; } ?>  >
                <td style="text-align:center;"><?php echo $val['serverId']; ?></td>
                <td style="text-align:center;"><?php echo $val['host']; ?></td>
                ...
                <td style="text-align:center;"><a href="?sevid=<?php echo $SevidCfg['sevid']?>'&mod=infomation&act=cache&key=<?php echo $k?>">删除</a></td>
            </tr>
        <?php } ?>
    </table>
<?php  }?>