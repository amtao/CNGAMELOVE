        ...
        <table style="width:100%;">
            ...
            if(!empty($volist)){
                foreach ($volist as $k => $v) {
                    ...
                    echo '<td>' . $v['time'] . '</td>';
                    echo '<td>' . ($v['login_pnum'] == 0? '0.00' : number_format($v['total_money']/$v['login_pnum'],2)) . '</td>';
                    echo '<td>' . ($v['login_pnum'] == 0?'0.00':number_format($v['total_rechange_pnum']/$v['login_pnum'],2)).'%' . '</td>';
                    echo '<td>' . $v['new_rechange_pnum'] . '</td>';
                    echo '<td>' . ($v['reg_pnum'] == 0?'0.00':number_format($v['new_rechange_pnum']/$v['reg_pnum'],2)).'%' . '</td>';
                    echo '<td>' . ($v['total_rechange_pnum'] == 0 ? '0.00' : (empty($v['total_rechange_pnum']) ? 0 :number_format($v['total_money']/$v['total_rechange_pnum'],2))) . '</td>';
                    echo '</tr>';
                }
            }
//            ?>
