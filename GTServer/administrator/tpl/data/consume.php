...
<table style="width: 100%">
    ...
    <tr>
        <th style="text-align:right;width: 100px;">统计类型：(全服查询不生效)</th>
        <td style="text-align:left;" colspan="3">
            <input type="radio" name="stype" value="1"<?php if(empty($_POST['stype']) || $_POST['stype'] == 1) echo "checked";?>/> 全部
            <input type="radio" name="stype" value="2"<?php if($_POST['stype'] == 2) echo "checked";?>/> 付费
            <input type="radio" name="stype" value="3"<?php if($_POST['stype'] == 3) echo "checked";?>/> 非付费
        </td>
    </tr>
    <tr>
        <th style="text-align:right;width: 100px;">道具类型：</th>
        <td style="text-align:left;" colspan="3">
            <input type="radio" name="itemId" value="1"<?php if(empty($_POST['itemId']) || $_POST['itemId'] == 1) echo "checked";?>/> 元宝
            <input type="radio" name="itemId" value="6"<?php if($_POST['itemId'] == 6) echo "checked";?>/> 金叶子
        </td>
    </tr>
    <tr>
...
<table style="width: 100%" id="tableId">
    <thead>
        <tr>
            <th colspan="3">消费统计</th>
        </tr>
        <tr>
            <th>消费类型</th>
            <th onclick="sortAble(this,'tableId',1, 1, 'int')" style="cursor:pointer">消费金额▼</th>
            <th>占比</th>
        </tr>
    </thead>
    <tbody>
        <?php if(!empty($consume)){  ?>
            <?php foreach ($consume as $key => $value){
                ...
                echo '<td style="text-align:center;">'.number_format($value['money']*100/$total, 2).'%</td>';
                echo '</tr>';
                } ?>
        <?php } ?>
    </tbody>
