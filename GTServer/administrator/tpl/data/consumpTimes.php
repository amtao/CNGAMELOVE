...
<table style="width: 100%" id="tableId">
    <thead>
        ...
        <tr>
            <th>消费类型</th>
            <th onclick="sortAble(this,'tableId',1, 1, 'int')" style="cursor:pointer">消费人次▼</th>
            <th>占比</th>
        </tr>
    </thead>
    <tbody>
        <?php if(!empty($consume)){  ?>
            <?php foreach ($consume as $key => $value){
                ...
                echo '<td style="text-align:center;">'.number_format($value['times']*100/$times, 2).'%</td>';
                echo '</tr>';
                } ?>
        <?php } ?>
    </tbody>
    <th style="text-align:center;">总计</th>
    <td style="text-align:center;"><?php echo $times; ?></td>
