...
<table>
    ...
    <tr>
        ...
        <td style="text-align:left;" colspan="3">
            <input type="radio" name="stype" value="1"<?php if(empty($_POST['stype']) || $_POST['stype'] == 1) echo "checked";?>/> 消费
            <input type="radio" name="stype" value="2"<?php if($_POST['stype'] == 2) echo "checked";?>/> 人次
            <input type="radio" name="stype" value="3"<?php if($_POST['stype'] == 3) echo "checked";?>/> 人数
        </td>
    </tr>
...
<?php if($dataInfo):?>
    ...
    <table style="width: 100%" id="tableId">
        ...
        <tr>
            ...
            <th>礼包档次(元)</th>
            <th>礼包内容</th>
            <th onclick="sortAble(this,'tableId',1, 2, 'int')" style="cursor:pointer"><?php if($_POST['stype'] == 1){echo '充值RMB(元)';}elseif ($_POST['stype'] == 2){echo '充值人次';}else{echo '充值人数';} ?></th>
            <th>占比</th>
        ...
        <?php foreach ($dataInfo as $key => $value){
            ...
            echo '<td style="text-align:center;">'.$rwd[$key]['name'].'</td>';
            echo '<td style="text-align:center;">'.$rwd[$key]['present'].'</td>';
            echo '<td style="text-align:center;">';

            foreach ($rwd[$key]['items'] as $keys => $values) {
                ...
                echo $items[$values['id']]['name_cn'];
                echo ' <b style="padding-left:10px;"> 数 量 : </b>'.$values['count'].'</span>';
            }

            echo '</td>';
            echo '<td style="text-align:center;">'.$value['rmb'].'</td>';
            echo '<td style="text-align:center;">'.number_format($value['rmb']*100/$total, 2).'%</td>';
            echo '</tr>';
        } ?>
        </tbody>
