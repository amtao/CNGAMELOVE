...
define('EXCEL_SERVER_CONFIG',PROJECT_ROOT.'01_design/ExcelServerPhp/');
require_once dirname( __FILE__ ) . '/../../public/common.inc.php';
Common::loadLib("PHPExcel/Classes/PHPExcel");
...
function get_excel_data($fileName,$getNum = 0){
    ...
    for($currentRow = 1 ; $currentRow <= $allRow ; $currentRow++ )
    {
        ...
        for($currentColumn=0;$currentColumn < $end_index;$currentColumn++){
            ...
            $row = $currentSheet->getCell($col_name.'1')->getCalculatedValue();
            ...
            if ($fileName == 'use_clothe.xls' && ($row == 'display' || $row == 'location')) {

            } else {
                if (is_string($zhi) && is_array(json_decode($zhi,true))) {
